...
// Encode dokumente
const docEmbeddings: number[][] = await Promise.all(
    documents.map(async (d) => Array.from((await embedder(d.text))[0].dataSync()))
);
...
const queryEmbedding: number[] = Array.from((await embedder(query))[0].dataSync());
